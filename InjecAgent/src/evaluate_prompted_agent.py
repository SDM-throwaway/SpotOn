...
def predict_one_case(system_prompt, user_prompt, params, item, model_class, tool_dict, step_2=False):
    ...
    if step_2:
        ...
        protected_tool_response = Defenses.randomized_datamarking(
        item['Tool Response']
    )
    ...
    agent_scratchpad = (
        ...
        f"\nAction: {item['User Tool']}"
        f"\nAction Input: {item['Tool Parameters']}"
        f"\nObservation: {protected_tool_response}\n"
    )
...
def main(params):
    ...
    safe_model_name = params['model_name'].replace(":", "_")
    output_dir = f"./results/prompted_{params['model_type']}_{safe_model_name}_{params['prompt_type']}"
