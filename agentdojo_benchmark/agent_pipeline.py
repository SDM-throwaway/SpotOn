...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        # ─────────────────────────────────────────────────────────────────────────────
        # The three defenses below follow the same pattern:
        #   1. Prepend the defense's instruction block to the system prompt so
        #      the model knows the convention used for untrusted tool output.
        #   2. Wrap every tool result with the corresponding Defenses method
        #      before it reaches the model.
        # ─────────────────────────────────────────────────────────────────────────────

        if config.defense == "delimiting":
            # Wraps tool output in <<<UNTRUSTED_DATA_START>>> / <<<UNTRUSTED_DATA_END>>> markers.
            system_message_component.system_message = (
                Defenses.delimiting_system_prompt() + "\n\n" + config.system_message
            )

            def delimiting_formatter(result):
                raw = tool_output_formatter(result)
                return Defenses.delimiting(raw)

            tools_loop = ToolsExecutionLoop([ToolsExecutor(delimiting_formatter), llm])
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline

        if config.defense == "datamarking":
            # Inserts a "^" character between every word of the tool output so
            # the model can distinguish data tokens from instruction tokens.
            # NOTE: best suited for plain-text tool responses; may garble
            # structured (JSON/YAML) output — prefer "delimiting" if your
            # tools return structured data.
            system_message_component.system_message = (
                Defenses.datamarking_system_prompt() + "\n\n" + config.system_message
            )

            def datamarking_formatter(result):
                raw = tool_output_formatter(result)
                return Defenses.datamarking(raw)

            tools_loop = ToolsExecutionLoop([ToolsExecutor(datamarking_formatter), llm])
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline

        if config.defense == "encoding":
            # Base64-encodes the entire tool output so injected instructions
            # cannot be parsed as natural language by the model.
            system_message_component.system_message = (
                Defenses.encoding_system_prompt() + "\n\n" + config.system_message
            )

            def encoding_formatter(result):
                raw = tool_output_formatter(result)
                return Defenses.encoding(raw)

            tools_loop = ToolsExecutionLoop([ToolsExecutor(encoding_formatter), llm])
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
