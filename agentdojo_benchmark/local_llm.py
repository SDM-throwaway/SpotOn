...
def _make_system_prompt(system_message: str, tools: Collection[Function], promt_counter: int) -> str:
    if len(tools) == 0:
...
class LocalLLM(BasePipelineElement):
    _prompt_counter: int = 0 # ugly fix
    ...
    def query(
        ...
        if self.model == "local":
            LocalLLM._prompt_counter += 1
            print(f"[local] prompt #{LocalLLM._prompt_counter}")
        messages_ = []
        count = 0
        for m in messages:
            ...
            if role == "system" and content is not None:
                content = _make_system_prompt(get_text_content_as_str(content), runtime.functions.values(), count)
            if role == "tool":
